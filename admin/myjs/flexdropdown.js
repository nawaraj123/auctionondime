...
var flexdropdownmenu={
	...
	animspeed: 20, //reveal animation speed (in milliseconds)
	...
	buildflexmenu:function($, $menu, $target){
		...
		$lis.each(function(i){
			...
			$li.children("a:eq(0)").append( //add arrow images
				'<img src="../../Dristi_ad_test/myjs/'+flexdropdownmenu.arrowpath+'" class="rightarrowclass" style="border:0;" />'
			)
	...
	init:function($, $target, $flexmenu){
		...
		if ($target.parents().filter('ul.jqflexmenu').length>0) //if $target matches an element within the flex menu markup, don't bind onflexmenu to that element
			return
		var useroffsets=$target.attr('data-offsets')? $target.attr('data-offsets').split(',') : [0,10] //get additional user offsets of menu
