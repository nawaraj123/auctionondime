     ...
     public function getFnews()
	  {
	    ...
	    public function setFnews($fnews)
	  {
	 $this->fnews=$fnews;
	  }
  ...
  private function createDestination(){
   ...
   function resize_both($name1,$location,$new_w,$new_h){
    ...
    if($ratio > $nratio)
   ...
   else
     {
	  ...
	  public function add(){
	       ...
	       $query="insert into tbl_testimonial(heading, description, detail, image, fnews) values('$this->heading', '$this->description', 
																		'$this->detail', '$destination', '$this->fnews')";
		   
		              mysql_query($query) or die("error on insert");	
			}else{
			
			$query="insert into tbl_testimonial(heading, description, detail,fnews) values('$this->heading', '$this->description', '$this->detail', '$this->fnews')";
	...
	public function getNews($id){
	      ...
	      $resultSearch= mysql_query($query);
			...
			if($resultSearch){
					...
					$news->setHeading($row['heading']);
					
					$news->setDescription($row['description']);
					
					$news->setDetail($row['detail']);
					
					$news->setImage($row['image']);
					$news->setFnews($row['fnews']);
	...
	public function getImages($id){
	      ...
	      $query="select image from tbl_testimonial where id=$id";
	...
	public function updateNews($id){
	   ...
	   $destination=$this->createDestination();
		 ...
		 if($destination){
			...
			$sel="select image from tbl_testimonial where id=$id";
			
			$result=mysql_query($sel);
			
			$data=mysql_fetch_assoc($result);
			
			$query="update tbl_testimonial set heading='$this->heading', description='$this->description', detail='$this->detail', image='$destination', fnews='$this->fnews' where id=$id";
			...
			unlink($data['image']);
		       ...
		       $query="update tbl_testimonial set heading='$this->heading', description='$this->description', detail='$this->detail', fnews='$this->fnews' where id=$id ";
	...
	public function delete(){
			...
			$sel="select image from tbl_testimonial where id='$del_id'";
			
			$result=mysql_query($sel);
			
			$data=mysql_fetch_assoc($result);
			
			$query="delete from tbl_testimonial where id='$del_id'";
		...
		public function deleteImage($id){
			...
			if($data['image']!=""){
			   ...
			   unlink($data['image']);
			      ...
			      $query="update tbl_testimonial set image='' where id=$id";
