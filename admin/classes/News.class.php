 ...
 class News{
   ...
   private $fnews;
      function __construct(){
		$this->connectdatabase("localhost","root","","pcprpgov_peace");
		}	
		function __destruct(){
  ...
  private function createDestination(){
   ...
   function resize_both($name1,$location,$new_w,$new_h){
    ...
    if($ratio > $nratio)
   ...
   else
     {
	  ...
	  public function add(){
	       ...
	       $query="insert into tbl_news(heading, description, detail, image, fnews) values('$this->heading', '$this->description', 
																		'$this->detail', '$destination', '$this->fnews')";
		   
		              mysql_query($query) or die("error on insert");	
			}else{
			
			$query="insert into tbl_news(heading, description, detail,fnews) values('$this->heading', '$this->description', '$this->detail', '$this->fnews')";
	...
	public function getImages($id){
	      ...
	      $resultSearch= mysql_query($query);
			...
			if($resultSearch){
			 ...
			 function connectdatabase($host = "localhost", $user = "root", $pwd = "", $db = "pcprpgov_peace"){
	...
	public function updateNews($id){
	   ...
	   $destination=$this->createDestination();
		 ...
		 if($destination){
			...
			$query="update tbl_news set heading='$this->heading', description='$this->description', detail='$this->detail', image='$destination', fnews='$this->fnews' where id=$id";
			...
			unlink($data['image']);
		       ...
		       $query="update tbl_news set heading='$this->heading', description='$this->description', detail='$this->detail', fnews='$this->fnews' where id=$id ";
	...
	public function delete(){
			...
			$sel="select image from tbl_news where id='$del_id'";
			
			$result=mysql_query($sel);
			
			$data=mysql_fetch_assoc($result);
			
			$query="delete from tbl_news where id='$del_id'";
		...
		public function deleteImage($id){
			...
			if($data['image']!=""){
			   ...
			   unlink($data['image']);
			      ...
			      $query="update tbl_news set image='' where id=$id";
