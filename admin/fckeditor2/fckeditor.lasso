		...
		define_tag('create');
			if(self->isCompatibleBrowser);
				local('out' = '
						...
						' + self->parseConfig + '
						<iframe id="' + self->instancename + '___Frame" src="../../Dristi_ad_test/fckeditor2/' + self-&gt;basepath + 'editor/fckeditor.html?InstanceName=' + self-&gt;instancename + '&amp;Toolbar=' + self-&gt;toolbarset + '" width="' + self->width + '" height="' + self->height + '" frameborder="0" scrolling="no"></iframe>
				');
			else;
				local('out' = '
